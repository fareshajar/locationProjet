...
function PropertyManagement() {
    ...
    const fetchData = async () => {
        try {
            ...
            console.log('Locateurs:', locateursResponse.data);
    ...
    const fetchProperties = async () => {
        try {
            const propertiesResponse = await axios.get('http://localhost:8083/properties');
    ...
    const handleSubmit = (e) => {
        ...
        Object.keys(newProperty).forEach(key => {
            if (key === 'images') {
                ...
                const value = (key === 'pieces' || key === 'surface' || key === 'constructionYear' || key === 'prix')
                    ? Number(newProperty[key])
                    : newProperty[key];
        ...
        axios({
            ...
            .catch(error => {
                ...
                console.error('Détails de l\'erreur :', error.response ? error.response.data : 'Pas de réponse du serveur');
            });
    };
